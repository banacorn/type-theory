...
module Entailment {j} (Judgement : Set j)  where
...
open import Membership Judgement public

-- The Derivability Judgement
infix 3 _⊢_[_]
data _⊢_[_] : List Judgement → Judgement → (_×_ : Judgement → Judgement → Judgement) → Set j where
    in-context : ∀ {J Γ R} → J ∈ Γ → Γ ⊢ J [ R ]
    by-rule : ∀ {Γ J K R} → Γ ⊢ J [ R ] → Γ ⊢ K [ R ] → Γ ⊢ R J K [ R ]

    -- by-rules   : ∀ {J Γ} → alg Γ  → Γ ⊢ J
